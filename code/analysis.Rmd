...
# ggsave("images/output/fff_trend.png")
```
...
# company_characteristics_2018 <- company_characteristics_2018 %>%
# filter(CTRY_OF_DOM_NAME %in% market_vect)

# View(company_characteristics_2018)
```
...
reg_data_w <- merge(company_characteristics_2018, data_abnormal_returns_w, by = "NAME")  %>%
    ...
    mutate(
        ...
        POST = ifelse(DATE >= as.Date("2019-09-13"), 1, 0),
...
ggplot() + geom_line(data = car_graph_w, aes(x = DATE, y = mean_car, color = as.factor(T_cont), group=T_cont))
...
reg_data_w_eve <- reg_data_w  %>%
    mutate(
        TimeTo_T = as.numeric(difftime(reg_data_w$DATE, event_date_2, units = "weeks"))
    )
...
ggsave("images/output/stock_d_trend.png")
```
...
print("------------")
model <- feols(CAR ~  TxPOST_2 | NAME_factor + DATE_factor,
...
model <- feols(CAR ~  i(T_cont, POST_2, ref = 1) | NAME_factor + DATE_factor,
               data = reg_data_d, cluster = ~ NAME_factor)
summary(model)
...
# ----------------
# event study test with the continuous case 
# ----------------

reg_data_d_cont <- reg_data_d_eve %>%
    mutate(
        T_cont = as.factor(T_cont)
    )

mod_twfe <- feols(CAR ~ i(TimeTo_T_2, T_cont, ref = -1) | DATE_factor + NAME_factor,
                  cluster = ~NAME_factor, data = reg_data_d_cont)
summary(mod_twfe)
```
...
ggsave("images/output/sales_trend.png")
...
# # event study
# mod_twfe_cont <- feols(LOG_SALES_DIFF_4 ~ i(TimeTo_Q, T_cont, ref = -0.25) | DATE_factor + NAME_factor,
#                   cluster = ~NAME_factor, data = reg_data_sales)

# summary(mod_twfe_cont)
