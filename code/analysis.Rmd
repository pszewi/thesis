...
eve_by_t <- function(object, t_group, title = "", x_axis = "", y_axis = ""){
    # make x_axis
    x_axis_data <- unique(as.numeric(str_extract(rownames(object$coeftable), "(?<=TimeTo_T::).*")))
    time <- seq(min(x_axis_data), max(x_axis_data), length = (length(x_axis_data)+1))
    ref <- time[!(time %in% x_axis_data)]
    ...
    plot_df <- cbind(x_axis_data, estimates, ci_h, ci_l)
    plot_df <- rbind(plot_df, c(ref, 0, NA, NA))
    plot_df <- plot_df[order(plot_df[,1]),]

    # make plot
    p <- ggplot(data = as.data.frame(plot_df)) + 
        geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
        geom_vline(xintercept = ref, linetype = "dashed", color = "grey50") +
        geom_errorbar(aes(x = time, ymin = ci_l, ymax = ci_h), 
             width = 0.02, color = "black") + 
        geom_point(aes(x = time, y = estimates), 
              size = 2, color = "black", shape = 19) +
        scale_x_continuous(breaks = unique(time)) +
...
ggsave("images/output/nyt_articles.pdf")
...
ggplot(data = fff_google) +
        geom_line(aes(x = week, y = value, group = 1), linewidth = 1, color = "black") +
        geom_point(data = subset(fff_google, is_spike), 
                             aes(x = week, y = value), 
                             color = "darkred", size = 2.5, shape = 19) +
...
ggsave("images/output/fff_trend.pdf")
```
...
ggplot(data = car_graph_d) +
    geom_line(aes(x = DATE, y = mean_car, color = as.factor(T_cont), group = T_cont), linewidth = 1) +
    geom_vline(aes(xintercept = as.Date("2019-09-20")),
               color = "red", linetype = "dashed", size = 0.8) +
    scale_color_grey(start = 0.2, end = 0.8, name = "Treatment Group") +
...
ggsave("images/output/stock_d_trend.pdf")
```
...
print("------------")
model <- feols(CAR ~  i(T_cont, POST_1, ref = 1) | NAME_factor + DATE_factor,
...
ggsave("images/output/sales_trend.pdf")
...
# making the plots 
sales_2 <- ggplotGrob(eve_by_t(sales_eve, "T_cont::2", "Group 2"))
sales_3 <- ggplotGrob(eve_by_t(sales_eve, "T_cont::3", "Group 3"))
sales_4 <- ggplotGrob(eve_by_t(sales_eve, "T_cont::4", "Group 4"))
plots = list(sales_2, sales_3, sales_4)


# First create a title grob with larger text
title <- textGrob("", gp=gpar(fontsize=18, fontface="bold"))
left <- textGrob("Estimate", gp=gpar(fontsize=16, fontface="bold"), rot=90)
bottom <- textGrob("Time To Event", gp=gpar(fontsize=16, fontface="bold"))

# Arrange the plots with proper margins for the labels
g <- marrangeGrob(grobs = plots, 
                 ...
                 padding = unit(2, "line"))
g
ggsave("images/output/sales_eve.pdf", g, height = 15)
